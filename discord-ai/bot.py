...
from ai_pipeline import answer_onboarding_question, clean_markdown_formatting, apply_draft_to_docs, process_message_batch as analyze_messages_and_generate_draft, extract_and_update_keywords, search_knowledge_base
...
async def on_message(message: discord.Message):
    ...
    if channel_name == ONBOARDING_CHANNEL_NAME:
        ...
        if extracted_kw or "?" in message.content or bot.user in message.mentions or any(k in message.content.lower() for k in ["how", "help", "setup", "where", "python", "env", "error", "fastapi", "db", "pip", "discord", "token", "run", "issue", "bot"]):
            ...
            if ai_reply:
                # Discord has a hard 2000 character limit per message.
                safe_reply = ai_reply[:1950] + ("...\n\n(Response truncated due to Discord length limits.)" if len(ai_reply) > 1950 else "")
                await message.reply(safe_reply)
...
async def post_draft_for_approval(draft_id: int, content: str, priority: str = "Medium", target_section: Optional[str] = None, proposed_change: Optional[str] = None, target_channel: Optional[Any] = None):
    ...
    if len(clean_content) > 1800:
        clean_content = clean_content[:1800] + "...\n\n(Draft truncated for Discord display)"
